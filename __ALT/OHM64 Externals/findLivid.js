/*
    ...
    Once you get a match on the first 8 bytes (but ignoring the 3rd byte), then the remainder will tell you which Livid device it is. � "fc" will be 0001 for any Livid product based on the USB-MIDI DIY brain. � "fn" will be 0002 for the OHM64. � For the OHM64 specifically, the bytes would be:
    F0h 7Eh chh 06h 02h 00h 01h 61h 01h 00h 02h 00h v1h v2h v3h v4h F7h
    (dec) 240 126 ch 6 2 0 1 97 1 0 2 0 v1h v2h v3h v4h 247
    You can match on the first 12 bytes (except the 3rd byte). � You never want to match on "v" - the version - since that may change. � However, once you collect those bytes from LSB to MSB, you'll have the same version as the USB Version XX.Y.Z, where XX is the major version, Y is the minor version, and Z is the revision or sub version. � Right now, we're looking at 00.1.9 as the next release, but the shipping version will be 01.0.0
    F0h 7Eh chh 06h 02h 00h 01h 61h 01h 00h 02h 00h 00h 00h 01h 00h F7h
    (dec) 240 126 ch 6 2 0 1 97 1 0 2 0 0 0 1 0 247
    "ch" will change depending upon what the user selects as the OHM64 basic channel. � That's why you have to keep ignoring the 3rd byte when matching. � Thus, you'll request an inquiry for channel 0x7F - the wild card to match all devices - but the response will never be a wild card, always a specific channel. 
*/
autowatch = 1;
outlets = 2;
...
var ch = 1;
var tomatch = [240,126,ch,6,2,0,1,97,1,0,productid,0,v1,v2,v3,v4]; // 6 2 means it's a reply to Inq. Req., 0 1 97 means Livid Mfg ID, 1 0 means USB-MIDI DIY brain, 2 0 means OHM64, 3 0 means Block
var proceed = 0;
var sxi=0;
...
function tryport(v){
    ...
    tout.schedule(8); //if we don't get a 240 in msg_int, we'll timeout and move on to the next port
}
...
function product(v){
    productid = v;
    tomatch = [240,126,ch,6,2,0,1,97,1,0,productid,0,v1,v2,v3,v4];
